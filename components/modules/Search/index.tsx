...
import { SearchIcon, ChevronDown, ArrowRight, SlidersHorizontal } from 'lucide-react-native';
            ...
            <View className='bg-background w-full h-full'>
                <View className='px-6 pt-16 pb-5'>
                    <View className="flex flex-row justify-between items-center mb-8">
                        ...
                        <Text className="block font-bold text-2xl text-foreground">Search</Text>
                        <Text></Text>
                    </View>
                                    ...
                                    <SelectDropdown
                                        ...
                                        renderButton={(selectedItem, isOpened) => (
                                            <View className="flex-row items-center gap-4 px-6 py-2 bg-secondary rounded-full">
                                                ...
                                                {isOpened ? <ChevronDown color='#fff' size={22} /> : <ChevronDown color='#fff' size={22} />}
                                            </View>
                                        )}
                                        renderItem={(selectedItem, item, isSelected) => {
                                            return <View
                                                className={`px-4 py-2 ${isSelected ? 'bg-secondary' : 'bg-accent '}`}
                                            >
                                                <Text className={`text-lg text-black ${isSelected ? "!text-background" : "text-foreground"}`}>{selectedItem}</Text>
                                            </View>
                                        }
                                        }
                                        dropdownStyle={{ borderRadius: 18, backgroundColor: '#f1f3f5', marginTop: -25 }}
                                        showsVerticalScrollIndicator={false}
                                    />
                                    <View className='h-[80%] bg-secondary w-0.5 ml-4 mt-2'>
                                    </View>
                                    <Button
                                        action="secondary"
                ...
                >
                    ...
                    <Text className="text-xl font-bold text-foreground px-6 mb-6">Category</Text>
                    ...
                    <View className='mb-6'>
                        <BottomSheetFlatList
                            ...
                            renderItem={({ item }) => (
                                ...
                                >
                                    <ButtonText className={`!text-lg leading-6 !text-primary !font-medium`}>
                                        {item.name}
                                    </ButtonText>
                                </Button>
                            )}
                    ...
                    <View className='px-6 mb-6'>
                        <Text className="text-xl font-bold text-foregroundmb-4 mb-6">Recipe Type</Text>
                        ...
                        <View className='flex flex-row flex-wrap justify-between '>
                            {recipesType?.map((rec, index) => {
                                ...
                                return <TouchableOpacity
                                    key={index}
                                    className={`basis-[31%] rounded-full bg-gray4 py-3 px-3 flex items-center justify-center mb-4 ${isSelected ? "bg-secondary" : "bg-gray4"}`}
                                    onPress={() => handleSelection(index)}
                                >
                                    <Text className={`text-center text-foreground leading-6 font-medium ${isSelected ? "!text-background" : "!text-foreground"}`}>{rec}</Text>
                    ...
                    <View className='px-6 mb-10'>
                        <View className='flex flex-row justify-between items-center mb-8'>
                            <Text className="text-xl font-bold text-foreground">Calories Range</Text>
